      ...
      class kernel_time_compute_nullhypot {
        ...
        public:
          ...
          ALPAKA_FN_ACC void operator()(TAcc const& acc,
                                        ...
                                        ScalarType* chi2s,
                                        ScalarType* sum0s,
                                        ScalarType* sumAAs) const {
            constexpr auto nsamples = EcalDataFrame::MAXSAMPLES;
            auto const nchannels = digisDevEB.size() + digisDevEE.size();  // FIXME maybe not needed to pass the digis if only the total number of channels is needed
      ...
      class kernel_time_compute_makeratio {
        ...
        public:
          ...
          ALPAKA_FN_ACC void operator()(TAcc const& acc,
            ...
            // constants
            constexpr uint32_t nthreads_per_channel = 45;  // n=10, n(n-1)/2
            constexpr auto nsamples = EcalDataFrame::MAXSAMPLES;
            ...
            auto const blockIdx = alpaka::getIdx<alpaka::Grid, alpaka::Blocks>(acc)[0u];
            auto const blockDim = alpaka::getWorkDiv<alpaka::Block, alpaka::Threads>(acc)[0u];
            auto const gtx = threadIdx + blockDim * blockIdx;
            auto const ch = gtx / nthreads_per_channel;
            auto const ltx = threadIdx % nthreads_per_channel;
            ...
            if (!condForUselessSamples) {
              ...
              if (!internalCondForSkipping1) {
                ...
                if (ratio_step == 1 && ratio_value >= l_timeFitLimits_first && ratio_value <= l_timeFitLimits_second) {
                  ...
                  // below.
                  shrTimeMax[threadIdx] = error2 > 0 ? time_max : 0;
                  shrTimeWgt[threadIdx] = error2 > 0 ? time_wgt : 0;
                } else {
                  shrTimeMax[threadIdx] = 0;
                  shrTimeWgt[threadIdx] = 0;
                }
                ...
                auto const rmax = std::min(ratio_value + ratio_error,
                                           fast_expf(static_cast<ScalarType>(ratio_step) / beta) - 0.001);
            ...
            while (iter >= 1) {
              if (ltx < iter)
                ...
                shr_chi2s[threadIdx] = oddElements && (ltx == iter - 1 && ltx > 0)
                                             ? shr_chi2s[threadIdx]
                                             : std::min(shr_chi2s[threadIdx], shr_chi2s[threadIdx + iter]);
            ...
            if (!condForUselessSamples && !internalCondForSkipping1 && !internalCondForSkipping2) {
              ...
              auto const chi2min = shr_chi2s[threadIdx - ltx];
              ...
              shr_time_wgt[threadIdx] = inverseSigmaSquared;
              shr_time_max[threadIdx] = tmax * inverseSigmaSquared;
            } else {
              shr_time_wgt[threadIdx] = 0;
              shr_time_max[threadIdx] = 0;
            }
            ...
            while (iter >= 1) {
              if (ltx < iter) {
                shr_time_wgt[threadIdx] = oddElements && (ltx == iter - 1 && ltx > 0)
                                                ? shr_time_wgt[threadIdx]
                                                : shr_time_wgt[threadIdx] + shr_time_wgt[threadIdx + iter];
                shr_time_max[threadIdx] = oddElements && (ltx == iter - 1 && ltx > 0)
                                                ? shr_time_max[threadIdx]
                                                : shr_time_max[threadIdx] + shr_time_max[threadIdx + iter];
                shrTimeMax[threadIdx] = oddElements && (ltx == iter - 1 && ltx > 0)
                                              ? shrTimeMax[threadIdx]
                                              : shrTimeMax[threadIdx] + shrTimeMax[threadIdx + iter];
                shrTimeWgt[threadIdx] = oddElements && (ltx == iter - 1 && ltx > 0)
                                              ? shrTimeWgt[threadIdx]
                                              : shrTimeWgt[threadIdx] + shrTimeWgt[threadIdx + iter];
              }
            ...
            if (ltx == 0) {
              auto const tmp_time_max = shr_time_max[threadIdx];
              auto const tmp_time_wgt = shr_time_wgt[threadIdx];
              ...
              g_accTimeMax[ch] = shrTimeMax[threadIdx];
              g_accTimeWgt[ch] = shrTimeWgt[threadIdx];
      ...
      class kernel_time_compute_findamplchi2_and_finish {
        ...
        public:
          ...
          ALPAKA_FN_ACC void operator()(TAcc const& acc,
                                        ...
                                        ConfigurationParameters::type const* amplitudeFitParametersEB,
                                        ConfigurationParameters::type const* amplitudeFitParametersEE) const {
          /// launch ctx parameters are
          ...
          //#define DEBUG_FINDAMPLCHI2_AND_FINISH
            ...
            auto const blockIdx = alpaka::getIdx<alpaka::Grid, alpaka::Blocks>(acc)[0u];
            auto const blockDim = alpaka::getWorkDiv<alpaka::Block, alpaka::Threads>(acc)[0u];
            auto const gtx = threadIdx + blockIdx * blockDim;
            ...
            if (state == TimeComputationState::NotFinished) {
              ...
              // store into shared mem
              shr_sumAf[threadIdx] = sumAf;
              shr_sumff[threadIdx] = sumff;
            } else {
              shr_sumAf[threadIdx] = 0;
              shr_sumff[threadIdx] = 0;
            }
            ...
            if (sample < 5) {
              shr_sumAf[threadIdx] += shr_sumAf[threadIdx + 5];
              shr_sumff[threadIdx] += shr_sumff[threadIdx + 5];
            }
            ...
            if (sample < 2) {
              ...
              shr_sumAf[threadIdx] += shr_sumAf[threadIdx + 2] + shr_sumAf[threadIdx + 3];
              shr_sumff[threadIdx] += shr_sumff[threadIdx + 2] + shr_sumff[threadIdx + 3];
            }
            ...
            if (sample == 0) {
              ...
              auto const sumff = shr_sumff[threadIdx] + shr_sumff[threadIdx + 1] - shr_sumff[threadIdx + 3];
              auto const sumAf = shr_sumAf[threadIdx] + shr_sumAf[threadIdx + 1] - shr_sumAf[threadIdx + 3];
      ...
      class kernel_time_compute_fixMGPAslew {
        ...
        public:
          ...
          ALPAKA_FN_ACC void operator()(TAcc const& acc,
            ...
            auto const* digis = ch >= offsetForInputs ? digisDevEE.data()->array.data() : digisDevEB.data()->array.data();
        ...
        public:
          ...
          ALPAKA_FN_ACC void operator()(TAcc const& acc,
            ...
            if (gainId0 == 1 && use_sample(sample_mask, 0)) {
              ...
              if (gainId1 == 1 && use_sample(sample_mask, 1) && std::abs(diff) < 3 * conditionsDev.pedestals_rms_x12()[hashedId]) {
                pedestal = (pedestal + static_cast<ScalarType>(adc1)) / 2.0;
                num = 2;
              }
            } else {
            ...
            if (!use_sample(sample_mask, sample)) {
              ...
              sample_value =
                  (static_cast<ScalarType>(adc) - mean_x1) * gain6Over1 * gain12Over6;
            ...
            shrSampleValues[threadIdx] = sample_value_error > 0 ? sample_value : std::numeric_limits<ScalarType>::min();
            shrSampleValueErrors[threadIdx] = sample_value_error;
            alpaka::syncBlockThreads(acc);

            // perform the reduction with min
            if (sample < 5) {
              // note, if equal -> we keep the value with lower sample as for cpu
              shrSampleValueErrors[threadIdx] = shrSampleValues[threadIdx] < shrSampleValues[threadIdx + 5]
                                                      ? shrSampleValueErrors[threadIdx + 5]
                                                      : shrSampleValueErrors[threadIdx];
              shrSampleValues[threadIdx] = std::max(shrSampleValues[threadIdx], shrSampleValues[threadIdx + 5]);
            }
            alpaka::syncBlockThreads(acc);

            // a bit of an overkill, but easier than to compare across 3 values
            if (sample < 3) {
              shrSampleValueErrors[threadIdx] = shrSampleValues[threadIdx] < shrSampleValues[threadIdx + 3]
                                                      ? shrSampleValueErrors[threadIdx + 3]
                                                      : shrSampleValueErrors[threadIdx];
              shrSampleValues[threadIdx] = std::max(shrSampleValues[threadIdx], shrSampleValues[threadIdx + 3]);
            }
            alpaka::syncBlockThreads(acc);

            if (sample < 2) {
              shrSampleValueErrors[threadIdx] = shrSampleValues[threadIdx] < shrSampleValues[threadIdx + 2]
                                                      ? shrSampleValueErrors[threadIdx + 2]
                                                      : shrSampleValueErrors[threadIdx];
              shrSampleValues[threadIdx] = std::max(shrSampleValues[threadIdx], shrSampleValues[threadIdx + 2]);
            }
            alpaka::syncBlockThreads(acc);

            if (sample == 0) {
              // we only need the max error
              auto const maxSampleValueError = shrSampleValues[threadIdx] < shrSampleValues[threadIdx + 1]
                                                   ? shrSampleValueErrors[threadIdx + 1]
                                                   : shrSampleValueErrors[threadIdx];
      ...
      class kernel_time_correction_and_finalize {
        ...
        public:
          ...
          ALPAKA_FN_ACC void operator()(TAcc const& acc,
            ...
            auto const* digis = gtx >= offsetForInputs ? digisDevEE.data()->array.data() : digisDevEB.data()->array.data();
            ...
            auto* const amplitudeBins = isBarrel ? conditionsDev.timeBiasCorrections_amplitude_EB().data() : conditionsDev.timeBiasCorrections_amplitude_EE().data();
            auto* const shiftBins = isBarrel ? conditionsDev.timeBiasCorrections_shift_EB().data() : conditionsDev.timeBiasCorrections_shift_EE().data();
            auto const amplitudeBinsSize = isBarrel ? conditionsDev.timeBiasCorrections_amplitude_EB().size() : conditionsDev.timeBiasCorrections_amplitude_EB().size();
